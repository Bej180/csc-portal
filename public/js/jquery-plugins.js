...
$.confirm = async function(content, obj) {
  ...
  obj = {accept: ()=>{}, style: 'info', reject: ()=>{}, acceptText: 'Accept', rejectText: 'Reject', title:'Confirm', type:'confirm', ...obj};
  
  const body = $("body");
  const confirmElement = $(`<div class="confirm-backdrop show confirm-${obj.style}"></div>`);
  ...
  tabindex="-1" role="dialog" aria-live="assertive" aria-modal="true"><div class="confirm-icon-wrapper"><span class="confirm-icon"><i class="fa-2x faIcon "></i></span></div><div class="confirm-header"><h2 ng-if="title" class="confirm-title"></h2><button type="button" class="confirm-close confirm-dismiss" aria-label="Close this dialog">×</button></div><div class="confirm-content"><div id="confirm-content" class="confirm-html-container"></div></div>
...
confirmContent.html(htmlContent.text().replace(/\*\*([^\*]+)\*\*/g, "<b>$1</b>"));
  ...
  confirmAccept.on('click', function(){
      ...
      const accepted = obj.accept.call(confirmElement);
